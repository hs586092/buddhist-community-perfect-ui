...
import { TempleReviewSection } from './TempleReviewSection';
...
interface BuddhistCommunityAppProps {
  className?: string;
}

export const BuddhistCommunityApp: React.FC<BuddhistCommunityAppProps> = ({
  className = ""
}) => {
  const [currentSection, setCurrentSection] = useState<'home' | 'reviews' | 'chat' | 'emotion'>('home');
  ...
  const handleLogin = () => {
    // 로그인 모달 또는 페이지로 이동
    // 임시로 기본값 사용 (실제로는 모달에서 입력받아야 함)
    alert('로그인 기능은 곧 구현됩니다 🙏');
  };

  const handleSignup = () => {
    // 회원가입 모달 또는 페이지로 이동
    // 임시로 기본값 사용 (실제로는 모달에서 입력받아야 함)
    alert('회원가입 기능은 곧 구현됩니다 🙏');
  };

  const renderCurrentSection = () => {
    switch (currentSection) {
      case 'reviews':
        return <TempleReviewSection user={user} onBack={() => setCurrentSection('home')} />;
      case 'chat':
        return <CommunityChat user={user} onBack={() => setCurrentSection('home')} />;
      case 'emotion':
        return <EmotionCheckin user={user} onBack={() => setCurrentSection('home')} />;
      default:
  ...
  const renderHomeSection = () => (
    <main className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
          ...
          {user ? (
            <div className="text-center">
              <p className="text-gray-600 mb-4">
                환영합니다, <span className="font-semibold text-blue-600">{user.username}</span>님 🙏
              </p>
            </div>
          ) : (
            <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
        {/* 법회 리뷰 */}
        <motion.div 
          className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 border border-gray-200/50 card-hover peaceful-fade cursor-pointer" 
        ...
        <motion.div 
          ...
          onClick={() => setCurrentSection('emotion')}
          whileHover={{ scale: 1.02 }}
          whileTap={{ scale: 0.98 }}
        >
      ...
      {/* 오늘의 마음챙김 */}
      <motion.div 
        className="mt-12 bg-gradient-to-r from-purple-500/10 to-blue-500/10 rounded-2xl p-8 text-center peaceful-fade" 
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen lotus-gradient flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">불교 커뮤니티를 준비하고 있습니다...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={`min-h-screen lotus-gradient ${className}`}>
      {/* 상단 네비게이션 */}
...
export default BuddhistCommunityApp;